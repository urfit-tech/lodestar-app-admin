...
import { orderMessages, commonMessages } from '../../../helpers/translation'
...
export type OrderStatusTabRef = {
  handleSubmit: () => Promise<void>
}

const OrderStatusTab = forwardRef<OrderStatusTabRef, {
  orderLogId: string,
    ...
    <div>
      ...
      >
        <div className="row">
          <div className="col-6">
            <Form.Item label={formatMessage(orderMessages.label.orderLogStatus)} name="type">
              <Select<string> defaultValue={type} value={type} onChange={e => setType(e as ModifyType)}>
                {[
                  { operation: 'paid', label: formatMessage({ id: 'order.label.hasPaid', defaultMessage: '已支付' }) },
                  {
                    operation: 'refunded',
                    label: formatMessage({ id: 'order.label.hasRefunded', defaultMessage: '已退款' }),
                  },
                  {
                    operation: 'expired',
                    label: formatMessage(commonMessages.status.orderExpired),
                  },
                ]
          ...
          <div className="col-6">
            <Form.Item
              label={formatMessage({ id: 'order.label.paidPrice', defaultMessage: '支付金額' })}
              name="price"
              rules={[
                formInstance => ({
